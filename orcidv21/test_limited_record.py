...
class LimitedRecord(OrcidBaseTest.OrcidBaseTest):
 ...
 #Test no information is returned using the public API
    ...
    def test_read_limited_record_with_20_public_api(self):
		...
		#Compare the body of the response to the saved file.        
		self.assertTrue(response_body.strip() == open('saved_recordsv21/empty_limited_record20.xml','r').read(), 'response_body: ' + response_body)
...
#Test no information is returned using a read-public token on the member API       
    def test_read_limited_record_with_12_public_token(self):
		...
		response_body = response.partition('</orcid-history>')[2]
		#Compare the body of the response to the saved file.        
		self.assertTrue(response_body.strip() == open('saved_recordsv21/empty_limited_record12.xml','r').read(), 'response_body: ' + response_body)
...
#Test an active read-limited token returns information as expected
    def test_read_limited_record_with_12_limited_token(self):
		...
		response_body = response.partition('</orcid-history>')[2]
		#Compare the body of the response to the saved file.        
		self.assertTrue(response_body.strip() == open('saved_recordsv21/limited_record12.xml','r').read(), 'response_body: ' + response_body)
    ...
    def test_read_limited_record_with_20_limited_token(self):
		...
		response_body = response.partition('</history:history>')[2]
		#Compare the body of the response to the saved file.        
		self.assertTrue(response_body.strip() == open('saved_recordsv21/limited_record20.xml','r').read(), 'response_body: ' + response_body)
    ...
    def test_read_limited_work_with_20_limited_token(self):
        ...
        #Compare the body of the response to the saved file.
        self.assertTrue(response_body.strip() == open('saved_recordsv21/limited_record_work.json','r').read(), 'response_body: ' + response_body)
    ...
    def test_read_limited_email_with_20_limited_token(self):
    	...
    	response = self.orcid_curl("https://api." + properties.test_server + "/v2.1/" + self.limited_orcid_id + "/email", curl_params)
       	response_body = response.partition('X-Frame-Options: DENY')[2]
       	#Compare the body of the response to the saved file.
       	self.assertTrue(response_body.strip() == open('saved_recordsv21/limited_record_email.json','r').read(), 'response_body: ' + response_body)
