...
class Member20ApiPostUpdate(OrcidBaseTest.OrcidBaseTest):
    ...
    def test_post_work(self):
        ...
        response = self.orcid_curl("https://api.qa.orcid.org/v2.0/%s/work" % self.orcid_id, curl_params)
        ...
        update_response = self.orcid_curl("https://api.qa.orcid.org/v2.0/%s/work/%d" % (self.orcid_id,int(putcode)), update_curl_params)
