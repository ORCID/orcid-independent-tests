...
class OauthOpenId(OrcidBaseTest.OrcidBaseTest):
    ...
    def setUp(self):
        ...
        self.first_obo_code = self.generate_auth_code(self.first_obo_id, self.first_obo_scope)
        ...
        self.second_obo_code = self.generate_auth_code(self.second_obo_id, self.second_obo_scope)
    ...
    def test_013_full_scope_post_work(self):
        ...
        if properties.type != "actions":
            ...
            response = self.orcid_curl("https://api.%s/v3.0/%s/work/%s" % (self.test_server, self.orcid_id, putcode.group(1)), curl_params_delete)
            ...
            read_response = self.orcid_curl('https://%s/oauth/revoke' % (self.test_server), curl_params_revoke)