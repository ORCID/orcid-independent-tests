...
class PublicApiReadSearch(OrcidBaseTest.OrcidBaseTest):
    ...
    # Test search for the record with the 2.0 api and check that it is returned
        ...
        self.assertTrue("http://" + properties.test_server + "/" + self.orcid_id in response, "Record not returned in search" + response)
    ...
    # Test search for orcid with the 3.0_rc2 public api and check it is returned
        self.assertIsNotNone(self.token,"No token generated")
        curl_params = ['-H', 'Content-Type: application/orcid+xml', '-H', 'Accept: application/xml', '-H', 'Authorization: Bearer ' + self.token]
        response = self.orcid_curl("https://pub." + properties.test_server + "/v3.0_rc2/search?q=" + self.seach_value, curl_params)
        self.assertTrue("https://" + properties.test_server + "/" + self.orcid_id in response, "Record not returned in search" + response)
    ...
    # Test read record with the public 3.0_rc2 api and check that it is returned
        self.assertIsNotNone(self.token,"No token generated")
        curl_params = ['-H', "Accept: application/xml", '-H', 'Authorization: Bearer ' + self.token, '-L', '-i', '-k', '-X', 'GET']
        response = self.orcid_curl("https://pub." + properties.test_server + "/v3.0_rc2/" + self.orcid_id + "/record", curl_params)
        self.assertTrue("https://" + properties.test_server + "/" + self.orcid_id in response, "Name not returned on " + response)
    ...
    # Test read record with the public 3.0 api and check that it is returned
        self.assertIsNotNone(self.token,"No token generated")
        curl_params = ['-H', "Accept: application/xml", '-H', 'Authorization: Bearer ' + self.token, '-L', '-i', '-k', '-X', 'GET']
        response = self.orcid_curl("https://pub." + properties.test_server + "/v3.0/" + self.orcid_id + "/record", curl_params)
        self.assertTrue("https://" + properties.test_server + "/" + self.orcid_id in response, "Name not returned on " + response)
